...
//
  ...
  LOG( "ASKHadron", pDEBUG )
    << "q.E() = " << q.E() << " kaon 3p " << pk << " nucleon 3p " << pN;
  ...
  // In q frame, kaon and nucleon have equal and opposite transverse momentum
  // Both will be rotated by phi_kq in a bit
  double pNt = -pkt;
  double pNl = sqrt(pN*pN - pNt*pNt);

  LOG( "ASKHadron", pDEBUG )
    << "q3 " << q3 << " kaon longitudinal " << pkl << " nucleon longitudinal " << pNl << " trans " << pkt;
  ...
  LOG( "ASKHadron", pDEBUG )
    << "Kaon w.r.t q px = " << kaon.x() << " py = " << kaon.y() << " pz = " << kaon.z();
  LOG( "ASKHadron", pDEBUG )
    << "Proton w.r.t q px = " << nucleon.x() << " py = " << nucleon.y() << " pz = " << nucleon.z();
  LOG( "ASKHadron", pDEBUG )
    << "total px = " << kaon.x() + nucleon.x() << " py = " << kaon.y() + nucleon.y() << " pz = " << kaon.z() + nucleon.z();

  // Pick a phi for the k-q plane relative to nu-l plane (random)
  // For the total cross section we will split this out
