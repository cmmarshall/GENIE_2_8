...
//  }
  ...
  double theta = TMath::ACos(kinematics.GetKV(kKVctl));
  double phikq = kinematics.GetKV(kKVphikq);

  LOG( "AtharSingleKaon", pDEBUG)
    << "Tlep = " << Tlep << " Tk = " << Tkaon << " theta lep = " << theta << " phi_kq = " << phikq;
  
  // Set lepton mass
  if      (leptonPDG==kPdgElectron) aml = kElectronMass;
  else if (leptonPDG==kPdgMuon )    aml = kMuonMass;
  else if (leptonPDG==kPdgTau )     aml = kTauMass;     // WARNING: Model assumes E<2 GeV
  ...
  // returning d4sigma/dtk dpl dcosthetal dphi_kq
