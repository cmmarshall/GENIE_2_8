...
/*
 ...
 Author: Costas Andreopoulos <costas.andreopoulos \at stfc.ac.uk>
         STFC, Rutherford Appleton Laboratory - March 11, 2004
...
{
  const double total = PionNucleonXSec(Epion, true);
  const double elastic = PionNucleonXSec(Epion, false);
  return (total - elastic);
}
//____________________________________________________________________________
double genie::utils::hadxs::berger::TotalPionNucleonXSec(double Epion)
{
  return PionNucleonXSec(Epion, true);
}
//____________________________________________________________________________
double genie::utils::hadxs::berger::PionNucleonXSec(double Epion, bool get_total)
{
  ...
  double Epion2 = TMath::Power(Epion,2);
  double ppi = TMath::Sqrt( TMath::Max(0.,Epion2-kPionMass2) );
  ...
  const double M_pi = kPionMass;
  const double M_p = kProtonMass;
  const double pi = kPi;
  ...
  double Wdel  = 1.232;
  //double Gdel  = 0.130; // In original code, but unused
  double arg1  = (Wdel*Wdel - (M_p + M_pi)*(M_p + M_pi))*(Wdel*Wdel - (M_p -M_pi)*(M_p -M_pi));
  if(arg1<=0) arg1 = 0.0;
  //double pp1   = TMath::Sqrt(arg1)/2.0/Wdel; // In original code, but unused
